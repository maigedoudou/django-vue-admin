...
export default {
  ...
  actions: {
    ...
    async closeLeft ({ state, commit, dispatch }, { pageSelect } = {}) {
      ...
      if (currentIndex > 0) {
        ...
        for (let i = state.opened.length - 1; i >= 0; i--) {
          if (state.opened[i].name === 'dept' || i >= currentIndex) {
            continue
          }
    ...
    async closeRight ({ state, commit, dispatch }, { pageSelect } = {}) {
      ...
      // 删除打开的页面 并在缓存设置中删除
      for (let i = state.opened.length - 1; i >= 0; i--) {
        if (state.opened[i].name === 'dept' || currentIndex >= i) {
          continue
        }
